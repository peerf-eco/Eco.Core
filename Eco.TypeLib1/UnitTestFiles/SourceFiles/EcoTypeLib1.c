...
void SaveListTypeLib_IEcoList(IEcoTypeLib1* pTypeLib);
void SaveListTypeLib_IEcoCalculatorX(IEcoTypeLib1* pTypeLib);
void SaveListTypeLib_IEcoCalculatorY(IEcoTypeLib1* pTypeLib);
void LoadAndInspectList(IEcoTypeLib1* pTypeLib, const UGUID* riid);
...
int16_t EcoMain(IEcoUnknown* pIUnk) {
    ...
    // SaveListTypeLib_IEcoList(pTypeLib);
    SaveListTypeLib_IEcoCalculatorX(pTypeLib);
    SaveListTypeLib_IEcoCalculatorY(pTypeLib);

    LoadAndInspectList(pTypeLib, &IID_IEcoList1);
...
void SaveListTypeLib_IEcoList(IEcoTypeLib1* pTypeLib) {
    ...
    pTypeLib->pVTbl->SaveFile(pTypeLib, UGUIDPtrToTypeLibFileName(&IID_IEcoList1), pDir);
...
void SaveListTypeLib_IEcoCalculatorX(IEcoTypeLib1* pTypeLib) {
    ...
    /* --- Описываем метод: int32_t Addition([in] int16_t a, [in] int16_t b) --- */
    ...
    /* --- Описываем метод: int16_t Subtraction([in] int16_t a, [in] int16_t b) --- */
    ...
    /* 5. Сохраняем через основной интерфейс (выполнит расчет смещений и запись в файл) */
    pTypeLib->pVTbl->SaveFile(pTypeLib, UGUIDPtrToTypeLibFileName(&IID_IEcoCalculatorX), pDir);
...
void SaveListTypeLib_IEcoCalculatorY(IEcoTypeLib1* pTypeLib) {
    ...
    /* --- Описываем метод: int32_t Multiplication([in] int16_t a, [in] int16_t b) --- */
    ...
    /* --- Описываем метод: int16_t Division([in] int16_t a, [in] int16_t b) --- */
    ...
    /* 5. Сохраняем через основной интерфейс (выполнит расчет смещений и запись в файл) */
    pTypeLib->pVTbl->SaveFile(pTypeLib, UGUIDPtrToTypeLibFileName(&IID_IEcoCalculatorY), pDir);
...
void LoadAndInspectList(IEcoTypeLib1* pTypeLib, const UGUID* riid) {
    ...
    if (pTypeLib->pVTbl->LoadFile(pTypeLib, UGUIDPtrToTypeLibFileName(riid), &pDir) == 0) {
